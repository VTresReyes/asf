...
const unsigned long WEIGHT_ALERT_INTERVAL = 5000;     // 5 seconds
const unsigned long CAMERA_DELAY = 180000;            // NEW: 3 minutes (180000 ms)

const unsigned long SCHEDULE_CHECK_INTERVAL = 60000;  // 30 seconds
const unsigned long WEIGHT_ALERT_INTERVAL = 5000;     // 5 seconds
...
void loop() {
  ...
  // Check weight alert every 5 seconds
  if (currentMillis - last_weight_alert >= WEIGHT_ALERT_INTERVAL) {
    last_weight_alert = currentMillis;
    float current_weight = get_current_weight();
    low_weight_alert(current_weight);
  }
...
bool check_schedule(int &amount, int &schedule_id) {
  ...
  if (code == 200) {
    ...
    if (dispense) {
      amount = doc["amount"] | 0; // Store weight in grams
...
void low_weight_alert(float weight) {
  // Read button state with debounce
  int button_state = digitalRead(RESET_BUTTON);
  
  if (button_state == LOW && (millis() - last_button_press > DEBOUNCE_DELAY)) {
  ...
  if (weight < weight_threshold) {
    // Reset acknowledgment if weight drops even lower
    if (weight < weight_threshold / 2) {
      alert_acknowledged = false;
    }
    
    // Only alert if not acknowledged
    ...
    // Weight is normal - reset everything
    digitalWrite(BUZZER_PIN, LOW);
    alert_acknowledged = false;  // Reset for next time
  }
}
...
bool dispense(int feed_amount) {
  ...
  // float initial_weight = 1000;  // For testing
  ...
  while (current_weight > target_weight) {
    ...
    // current_weight = current_weight - 10;  // For testing
    
    // Optional: Print progress every 50g
    static float last_print_weight = initial_weight;
    if (initial_weight - current_weight >= last_print_weight - initial_weight + 50) {
      Serial.print("Progress: ");
      Serial.print(initial_weight - current_weight, 1);
      Serial.println(" g dispensed");
      last_print_weight = current_weight;
    }
  }
